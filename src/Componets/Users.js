...
const Users = () => {
  ...
  useEffect(() => {
    const filteredData = records.filter((user) => {
  ...
  const fetchData = async (page = 1, pageSize = 10) => {
    try {
      ...
      if (Array.isArray(response.data)) {
        ...
        const uniqueDesignations = [...new Set(response.data.map((user) => user.Designation_name))];
            ...
            <div className='container'>
              <Form layout='inline'>
                <Row gutter={[16, 16]}>
                  ...
                  <Col xs={24} sm={12} md={8} lg={6}>
                  <Form.Item name={'Designation'} label='Designation' style={{ maxWidth: '500px' }}>
                            <Select
                            ...
                            >
                              ...
                              {getDropdownOptions(records, 'Designation_name').map((option, index) => (
